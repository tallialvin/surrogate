...
from dataset_m2v import GraphDataset
...
parser.add_argument('--case_num', type=int, default=8, help='The case num')
parser.add_argument('--num_trains', type=int, nargs='+', default=[2000, 1000, 500, 100], help='Train data number')
...
def generate_folder_name(case_num, base_path):
    folder_base = f"cs{case_num}"
    folder = folder_base
    counter = 1
    
    while os.path.exists(os.path.join(base_path, folder)):
        folder = f"{folder_base}-{counter}"
        counter += 1
    
    return folder

def setup_experiment(case_num):
    base_folder = "experiment_results_m2v_100/multiple_object"
    ...
    return base_folder, folder, full_path

# Usage
case_num = args.case_num
base_folder, folder, full_path = setup_experiment(case_num)
base_folder = full_path
...
# case_num = 1
# base_folder = "experiment_results_m2n/multipele_object/cs1-1"
# folder = "cs1"
...
test_set, train_sets, val_sets = random_split(dataset, [num_test, num_training, len(dataset)-(num_test + num_training)])
test_loader = DataLoader(test_set, batch_size=1, shuffle=False)

# num_trains = [2000, 1000, 500, 100]
# num_trains = [20, 30, 40, 50]
...
for num_training in num_trains:
    ...
    # logging.basicConfig(filename=f"{model_folder}/accuracy_log.txt", level=logging.INFO)
