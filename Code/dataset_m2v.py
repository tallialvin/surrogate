...
class GraphDataset(Dataset):
    def __init__(self, directory, case_num, folder):
        ...
        with tqdm(total=len(self.file_numbers), desc="Processing files", unit="file") as pbar:
            while self.file_numbers:  # Continue until the list is empty
                ...
                file_path = os.path.join(self.directory, 'pickle', 'm2v', self.folder, f"data-set-{file_number}.pkl")
    ...
    def get_available_files(self):
        file_pattern = os.path.join(self.directory, 'pickle', 'm2v', self.folder, 'data-set-*.pkl')
    ...
    def process_file(self, file_number):
        ...
        if data is not None:
            ...
            x = torch.tensor([[
                                ...
                                1 if G.nodes[n]['graph_type'] == ('object' or 'target') else 0
                                ] 
                                for n in G.nodes()], 
                                dtype=torch.float)
