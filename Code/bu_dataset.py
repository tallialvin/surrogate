...
from shortpath2 import array_to_graph, knn_to_graph
from downsampling import downsample_cloud, farthest_point_sampling, no_sampling
...
class GraphDataset(Dataset):
    ...
    def process_file(self, file_number):
        ...
        if data is not None:
            ...
            # # Check the structure of node attributes
            # for node in G.nodes():
            #     node_data = G.nodes[node]
            #     print(f"Node {node} attributes:")
            #     print(f"  Position: {node_data['position']}")
            #     print(f"  Other data: {node_data['other_data']}")
            #     print(f"  Total number of top-level attributes: {len(node_data)}")
            #     print(f"  Number of position values: {len(node_data['position'])}")
            #     break  # Print only the first node for brevity

            # # Check the shape of the feature tensor
