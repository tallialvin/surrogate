...
def evaluate_mlp(train_loader, val_loader, model, device):
    ...
    for i in range(len(train_embeddings)):
        pred_prob = pred_model(Variable(torch.Tensor(train_embeddings[i]).to(device), requires_grad=False))
        # print(pred_prob)
        pred_prob = torch.unsqueeze(pred_prob, 0).to(device)
...
for iter in range(args.iterations):
    ...
    save_history_results(epoch_results, "third_model_(mlp+bestpar)", num_iter + 1,model)
