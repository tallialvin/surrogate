...
from store_result import save_epoch_results
...
for num_epoch in num_epochs:
    ...
    for iter in range(args.iterations):
        ...
        save_epoch_results(epoch_results, "hyper_epoch_"+str(num_epoch), num_iter + 1,model)

        num_iter +=1
        # Evaluate the results
        # test_result = evaluate(training_set, test_set, model, name='Test', max_num_examples=100)
        test_result = evaluate(training_set, test_set, model, device)
        test_accs.append(test_result['acc'])

        # val_result = evaluate(training_set, validation_set, model, name='Validation', max_num_examples=100)
        val_result = evaluate(training_set, validation_set, model, device)
        val_accs.append(val_result['acc'])

        print(str(test_result['acc']),str(val_result['acc']))
...
for batch_size in batch_sizes:
    ...
    for iter in range(args.iterations):
        ...
        save_epoch_results(epoch_results, "hyper_batch_"+str(batch_size), num_iter + 1,model)
...
nhid_sizes=[16,32,64,128,256]
for nhid_size in nhid_sizes:
    ...
    for iter in range(args.iterations):
        ...
        val_loader = DataLoader(validation_set, batch_size=args.batch_size, shuffle=False)
        test_loader = DataLoader(test_set, batch_size=1, shuffle=False)
        model = Net(num_features, nhid_size, args.final_dim, args.pooling_ratio, args.dropout_ratio).to(device)
        ...
        save_epoch_results(epoch_results, "hyper_nhid_"+str(nhid_size), num_iter + 1,model)
